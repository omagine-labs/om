...
function LoginForm() {
  ...
  // Detect if this is a desktop app sign-in request
  ...
  // Helper function to claim anonymous meetings after login
    ...
    const checkExistingAuth = async () => {
      ...
      try {
        ...
        if (user) {
          ...
          console.log(
            '[Login] User already authenticated, redirecting to success page'
          );
          router.replace('/desktop-success');
          return;
        }
      } catch (err) {
        console.error('[Login] Error checking existing auth:', err);
      }
    };

    checkExistingAuth();
  }, [isDesktopRequest, router]);

  // Capture UTM parameters on page load (same as signup page)
  useEffect(() => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (data && data.user) {
        // If desktop request, redirect to success page
        if (isDesktopRequest) {
          console.log(
            '[Login] Desktop sign-in successful, redirecting to success page'
          );
          router.replace('/desktop-success');
          return;
        }
        ...
        const hasClaimParams =
          meetingId || searchParams.get('speaker') || searchParams.get('email');

        // Try to claim anonymous meetings if params exist (non-blocking)
        if (hasClaimParams) {
          const claimResult = await claimAnonymousMeetings(
            data.user.id,
        ...
        // No meetings to claim, redirect to dashboard
  ...
  const handleMicrosoftSignIn = async () => {
    ...
    try {
      ...
      if (microsoftError) {
        setError(microsoftError.message || 'Failed to sign in with Microsoft');
        setLoading(false);
        return;
      }
          ...
          >
            ...
            <div className="mt-8 text-center">
              <p className="text-base text-slate-600">
                ...
                <Link
                  href={isDesktopRequest ? '/signup?source=desktop' : '/signup'}
