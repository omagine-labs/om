...
/**
 * Download a file from a URL using blob fetch for cross-origin compatibility.
 *
 * Fetches the file as a blob, creates a temporary object URL, and triggers
 * a download. Falls back to opening in a new tab if download fails.
 *
 ...
 * @example
 * ```typescript
 * await downloadFile('https://example.com/video.mp4', 'my-video.mp4');
 * ```
 */
