...
export default function WeeklyMetricsOverview() {
  ...
  const fetchWeeklyMetrics = async () => {
    try {
      ...
      // Fetch all speaker records where user is identified as a speaker from past 7 days
      ...
      const { data: rawRecords, error } = await supabase
        ...
        .select(
          ...
          communication_tips,
          created_at,
          meetings!inner(user_speaker_label)
        `
        )
      ...
      // Filter to only include records where speaker_label matches user_speaker_label
      const analysisRecords = rawRecords?.filter(
        (r: any) => r.speaker_label === r.meetings?.user_speaker_label
      );
