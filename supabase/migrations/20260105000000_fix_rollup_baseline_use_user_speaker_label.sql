-- Fix: Use user_speaker_label instead of assigned_user_id for rollup and baseline calculations
--
-- Problem: The rollup and baseline functions use `ma.assigned_user_id = p_user_id` to find
-- the user's speaker data. However, assigned_user_id is only set when confidence meets
-- threshold (60%/85%), while user_speaker_label is always set when the user is identified.
--
-- Solution: Change all queries to use:
--   WHERE m.user_id = p_user_id AND m.user_speaker_label = ma.speaker_label
-- This finds meetings owned by the user where they've been identified as a speaker.

-- ============================================================================
-- STEP 1: Update calculate_user_weekly_rollup function
-- ============================================================================
...
BEGIN
  ...
  -- Changed: Use user_speaker_label instead of assigned_user_id
  ...
  WHERE m.user_id = p_user_id
    AND m.user_speaker_label = ma.speaker_label
...
COMMENT ON FUNCTION public.calculate_user_weekly_rollup IS 'Calculates weekly rollup for a user based on meetings where user_speaker_label matches speaker. Deletes rollup if no meetings found.';

-- ============================================================================
-- STEP 2: Update calculate_initial_baseline function
-- ============================================================================
...
COMMENT ON FUNCTION public.calculate_initial_baseline IS 'Calculates initial baseline from first 5 meetings where user_speaker_label matches speaker.';

-- ============================================================================
-- STEP 3: Update update_current_baseline function
-- ============================================================================
...
COMMENT ON FUNCTION public.update_current_baseline IS 'Update rolling 12-week current baseline using user_speaker_label to find user meetings.';
