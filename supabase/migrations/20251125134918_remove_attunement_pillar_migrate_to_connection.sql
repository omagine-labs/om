...
-- Reason: Removing deprecated Attunement pillar and migrating functionality to Connection pillar
-- Impact: Drops collaboration_score, collaboration_explanation, attunement_pillar_score from 3 tables
-- Justification: Product decision to consolidate pillars from 4 to 3. Connection pillar now uses attunement_score instead of collaboration_score.
--
-- Migration: Remove Attunement Pillar and Migrate to Connection Pillar
--
-- Changes:
-- 1. Drop collaboration_score and collaboration_explanation from meeting_analysis
-- 2. Drop attunement_pillar_score from meeting_analysis
-- 3. Drop avg_collaboration_score and weekly_attunement_pillar_score from user_weekly_rollups
-- 4. Drop baseline_collaboration_score and avg_baseline_attunement_pillar_score from user_baselines
-- 5. Update weekly rollup function: Connection pillar now uses attunement_score
-- 6. Update baseline functions: Remove attunement pillar calculations
-- 7. Recalculate all historical weekly rollups and baselines

BEGIN;

-- ============================================================================
-- STEP 1: Drop columns from meeting_analysis
-- ============================================================================

ALTER TABLE public.meeting_analysis
DROP COLUMN IF EXISTS collaboration_score,
DROP COLUMN IF EXISTS collaboration_explanation,
DROP COLUMN IF EXISTS attunement_pillar_score;

COMMENT ON TABLE public.meeting_analysis IS 'Meeting analysis results with speaker-level metrics. Connection pillar now uses attunement_score.';

-- ============================================================================
-- STEP 2: Drop columns from user_weekly_rollups
-- ============================================================================

ALTER TABLE public.user_weekly_rollups
DROP COLUMN IF EXISTS avg_collaboration_score,
DROP COLUMN IF EXISTS weekly_attunement_pillar_score;

-- ============================================================================
-- STEP 3: Drop columns from user_baselines
-- ============================================================================

ALTER TABLE public.user_baselines
DROP COLUMN IF EXISTS baseline_collaboration_score,
DROP COLUMN IF EXISTS avg_baseline_attunement_pillar_score;

-- ============================================================================
-- STEP 4: Update calculate_user_weekly_rollup function
-- Connection pillar now uses attunement_score (no longer collaboration_score)
-- Attunement pillar calculation is removed
-- ============================================================================
...
COMMENT ON FUNCTION public.calculate_user_weekly_rollup IS 'Calculates weekly performance rollup for a user. Connection pillar now uses attunement_score.';

-- ============================================================================
-- STEP 5: Update calculate_initial_baseline function
-- Remove attunement pillar, keep Connection pillar with attunement_score
-- ============================================================================
...
BEGIN
  ...
  aggregated AS (
    SELECT
      ...
      -- Connection: avg_attunement × turn_taking_multiplier - interruptions_penalty
      -- CHANGED: Now uses attunement_score instead of collaboration_score
      GREATEST(0, LEAST(10,
        AVG(attunement_score) *
        CASE
...
COMMENT ON FUNCTION public.calculate_initial_baseline IS 'Calculates initial baseline from first 5 meetings. Connection pillar uses attunement_score.';

-- ============================================================================
-- STEP 6: Update update_current_baseline function
-- Remove attunement pillar from baseline calculations
-- ============================================================================
...
COMMENT ON FUNCTION public.update_current_baseline IS 'Update rolling 12-week current baseline. Connection pillar averaged from weekly rollups.';

-- ============================================================================
-- STEP 7: Recalculate all historical data
-- ============================================================================

-- Recalculate all weekly rollups for all users
DO $$
DECLARE
  user_record RECORD;
  week_record RECORD;
...
BEGIN
  RAISE NOTICE 'Starting recalculation of all weekly rollups...';
  ...
  LOOP
    -- For each week that has meetings
    FOR week_record IN
      SELECT DISTINCT DATE_TRUNC('week', m.start_time)::date as week_start
      FROM public.meeting_analysis ma
      JOIN public.meetings m ON ma.meeting_id = m.id
      WHERE ma.assigned_user_id = user_record.assigned_user_id
      ORDER BY week_start
    LOOP
      PERFORM public.calculate_user_weekly_rollup(
        user_record.assigned_user_id,
        week_record.week_start
      );
      recalc_count := recalc_count + 1;
    END LOOP;

    -- Recalculate baselines for this user
    PERFORM public.calculate_initial_baseline(user_record.assigned_user_id);
    PERFORM public.update_current_baseline(user_record.assigned_user_id);

    RAISE NOTICE 'Recalculated data for user: %', user_record.assigned_user_id;
  END LOOP;

  RAISE NOTICE 'Completed recalculation of % weekly rollups', recalc_count;
END $$;
