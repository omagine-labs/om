-- Migration: Fix weekly rollup to use recording_duration_seconds instead of end_time - start_time
-- Description: The calculate_user_weekly_rollup function was calculating meeting duration
--              using (end_time - start_time), but end_time is often NULL for uploaded recordings.
--              Instead, we should use recording_duration_seconds which is populated during
--              video processing. This also handles the case where meetings don't have an end_time.

BEGIN;

-- ============================================================================
-- Update calculate_user_weekly_rollup to use recording_duration_seconds
-- ============================================================================
...
BEGIN
  ...
  -- FIX: Use recording_duration_seconds instead of (end_time - start_time)
  ...
  SELECT
    ...
    -- Use recording_duration_seconds instead of end_time - start_time
    COALESCE(SUM(m.recording_duration_seconds), 0),
    COALESCE(AVG(m.recording_duration_seconds), 0),
...
COMMENT ON FUNCTION public.calculate_user_weekly_rollup IS 'Calculates weekly performance rollup for a user using recording_duration_seconds instead of end_time - start_time';
