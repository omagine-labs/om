...
import { reprocessMeeting } from '@/lib/reprocess';
...
interface MeetingsAndRecordingsProps {
  ...
  onIdentifySpeakerComplete?: (meetingId: string) => void;
...
export interface MeetingsAndRecordingsRef {
  ...
  addMeetingOptimistic: (meeting: Meeting) => void;
...
>(function MeetingsAndRecordings(
  ...
  const handleAssignmentComplete = (
    ...
    // Notify parent to navigate to analysis page
    onIdentifySpeakerComplete?.(meetingId);
  };
  ...
  const handleUnifiedModalSubmit = async (
    ...
    try {
      ...
      if (selectedMeetingForUpload) {
        ...
        // Update meeting details if they changed via IPC
        await meetingsApi.updateMeeting(selectedMeetingForUpload.id, {
          title: meeting.title,
      ...
      if (result.success) {
        ...
        // Poll for auto-deletion (happens when no speech detected in recording)
        {
          ...
          const checkInterval = setInterval(async () => {
            ...
            // Directly check database via IPC instead of full reload to avoid UI glitching
            const existsResult = await meetingsApi.meetingExists(meetingId);
              ...
              <div key={group.dateKey}>
                ...
                <div className="space-y-3">
                  {group.meetings.map((meetingWithRecording) => {
                    const isUploading =
                      uploadingMeetingId === meetingWithRecording.meeting.id;
                    const isDragOver =
                    ...
                    return (
                      <MeetingCard
                        ...
                        onDrop={
                          canUpload
                            ? (e) => handleDrop(e, meetingWithRecording.meeting)
                            : undefined
                        }
                      />
                    );
