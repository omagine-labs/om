...
export function useSpeakerIdentityGrouping({
  ...
  return useMemo(() => {
    ...
    const sortedGroups = Array.from(identityGroups.entries()).sort(
      ([_identityA, recordsA], [_identityB, recordsB]) => {
    ...
    return sortedGroups.map(([identity, records]) => {
      ...
      const recordsWithCollaboration = records.filter(
        (r) => r.collaboration_score != null
      );
      const avgCollaborationScore =
        recordsWithCollaboration.length > 0
          ? Math.round(
              recordsWithCollaboration.reduce(
                (sum, r) => sum + (r.collaboration_score || 0),
                0
              ) / recordsWithCollaboration.length
            )
          : null;
      const collaborationExplanations = records
        .map((r) => r.collaboration_explanation)
        .filter((exp): exp is string => Boolean(exp));
        ...
        metrics: {
          ...
          confidence_explanation:
            confidenceExplanations.length > 0
              ...
              : null,
          collaboration_score: avgCollaborationScore,
          collaboration_explanation:
            collaborationExplanations.length > 0
              ? collaborationExplanations.join(' ')
              : null,
