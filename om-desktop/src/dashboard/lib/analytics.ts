...
import { authApi, analyticsClientApi } from './api-client';
...
export async function trackEvent(
  ...
  try {
    ...
    // 2. Log to Supabase if user is authenticated (via IPC)
    try {
      // Get current user from main process
      const user = await authApi.getCurrentUser();

      if (user) {
        // Log event via IPC to main process
        const result = await analyticsClientApi.logEvent(
          user.id,
          eventName,
          properties
        );
        ...
        if (!result.success) {
          console.error(
            '[Analytics] Failed to log event to database:',
            result.error
          );
  ...
  if (!knownEvents.includes(eventName as any)) {
    console.warn(
      `[Analytics] Unknown event: "${eventName}". This event is not in the AARRR taxonomy. ` +
        `Please add it to types/analytics.ts or check for typos.`
    );
  }
...
export function resetAnalytics(): void {
  posthog.reset();
  // Note: Intercom integration removed for desktop app
}
