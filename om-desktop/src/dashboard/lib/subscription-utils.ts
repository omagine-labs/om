...
/**
 * Check if user is on a free account (internal_free plan or 100% discount)
 * A true free account has a permanent 100% discount, not just a $0 upcoming charge
 */
export function isFreeAccount(
  subscription: NonNullable<SubscriptionResponse['subscription']>
): boolean {
  // Check for internal_free plan type (used for internal team members)
  if (subscription.planType === 'internal_free') {
    return true;
  }
